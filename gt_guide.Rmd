...
First we'll run a linear regression with `lm()` using party to describe end attitude. Put the regression into `tbl_regression()` to get a preliminary visualization of the regression:
```{r regression}
...
# note for Carine: democrats have attitude of 8.78 and Republicans' atts increase by 2.17
...
obj <- stan_glm(data = trains, att_end ~ gender, refresh = 0)
```
...
```{r}
print(obj, detail = FALSE, digits = 1)
```
